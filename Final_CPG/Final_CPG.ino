...
//#define SPALLA_START_POSITION 1948 // Posizione iniziale di 180°
//#define GOMITO_START_POSITION 2048 // Posizione iniziale di 180°
#define OFFSET_SPALLA 285*2 // Oscillazione angolo spalla di 25°
#define OFFSET_GOMITO 341*2 // Oscillazione angolo gomito di 30°
#define N_CAMP 5 // Numero di campioni da scartare 
#define GAIT 3 // Tipo di gait da eseguire 1 Camminata Lenta , 2 Trotto , 3 Galoppo
...
int SPALLA_START_POSITION[] = {2120,2128,2015,2148};
int GOMITO_START_POSITION[] = {2048,2048,1948,2048};
...
void raw_angles(int i,float x) {
  angoli[i][0] = SPALLA_START_POSITION[i] - (x / 2.3) * OFFSET_SPALLA;
  angoli[i][1] = GOMITO_START_POSITION[i] - (x / 2.3) * OFFSET_GOMITO;
}
...
void setup() 
{
  ...
  for(int i=0; i<4;i++){
        ...
        Legs[i].servo(ID_SPALLA,SPALLA_START_POSITION[i]*x,0x3FF);
        Legs[i].servo(ID_GOMITO,GOMITO_START_POSITION[i]*x,0x3FF);
   }
   delay(10000);
}
...
void loop() {
  int x;
    ...
    for(int i=0; i<4;i++){
          ...
          Legs[i].servo(ID_SPALLA,angoli[i][0]*x,0x3FF);
          Legs[i].servo(ID_GOMITO,angoli[i][1]*x,0x3FF);
