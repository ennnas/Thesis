...
DynamixelStatus DynamixelInterface::multiwrite(uint8_t sID, uint8_t gID, uint8_t aAddress, uint8_t aSize, const uint8_t *sPtr, const uint8_t *gPtr, uint8_t aStatusReturnLevel)
{
	mPacket=DynamixelPacket(sID, DYN_WRITE, aSize+3, sPtr, aAddress);
	transaction(aStatusReturnLevel>1 && sID!=BROADCAST_ID);
	mPacket=DynamixelPacket(gID, DYN_WRITE, aSize+3, gPtr, aAddress);
	transaction(aStatusReturnLevel>1 && gID!=BROADCAST_ID);
	return mPacket.mStatus;
}
