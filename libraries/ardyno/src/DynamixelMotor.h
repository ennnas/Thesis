...
class DynamixelDevice
{
	...
	template<class T>
	inline DynamixelStatus multiwrite(uint8_t aAddress, const T& sData, const T& gData)
	{
		return mStatus=mInterface.multiwrite<T>(mID,mID+1, aAddress, sData, gData, mStatusReturnLevel);
	}
...
class DynamixelMotor:public DynamixelDevice
{
	...
	void multigoalPosition(uint16_t sPosition, uint16_t gPosition);
