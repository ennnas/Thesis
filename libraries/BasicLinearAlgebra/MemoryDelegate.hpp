...
template<int rows, int cols, class ElemT, class MemT> class Matrice;
...
Matrice<rows,cols,retElemT,Array<rows,cols,retElemT> > &Add(const Matrice<rows,cols,ElemT,Array<rows,cols,ElemT> > &A, const Matrice<rows,cols,opElemT,Array<rows,cols,opElemT> > &B, Matrice<rows,cols,retElemT,Array<rows,cols,retElemT> > &C)
{
...
template<int rows, int cols, class ElemT, class opElemT, class retElemT>
Matrice<rows,cols,retElemT,Array<rows,cols,retElemT> > &Subtract(const Matrice<rows,cols,ElemT,Array<rows,cols,ElemT> > &A, const Matrice<rows,cols,opElemT,Array<rows,cols,opElemT> > &B, Matrice<rows,cols,retElemT,Array<rows,cols,retElemT> > &C)
{
    for(int i = 0; i < rows; i++)
        for(int j = 0; j < cols; j++)
            C.delegate.m[i * cols + j] = A.delegate.m[i * cols + j] - B.delegate.m[i * cols + j];
...
Matrice<rows,operandCols,retElemT,Array<rows,operandCols,retElemT> > &Multiply(const Matrice<rows,cols,ElemT,Array<rows,cols,ElemT> > &A, const Matrice<cols,operandCols,opElemT,Array<cols,operandCols,opElemT> > &B, Matrice<rows,operandCols,retElemT,Array<rows,operandCols,retElemT> > &C)
{
    int i,j,k;
...
template<int rows, int cols, class ParentMemT, class ElemT = float> using RefMatrix = Matrice<rows, cols, ElemT, Reference<ElemT,ParentMemT> >;
...
template<int rows, int cols, class ElemT = float> using Identity = Matrice<rows, cols, ElemT, Iden<ElemT> >;
...
template<int rows, int cols = 1, class ElemT = float> using Zeros = Matrice<rows, cols, ElemT, Zero<ElemT> >;
...
template<int rows, int cols, int tableSize = cols, class ElemT = float> using SparseMatrix = Matrice<rows, cols, ElemT, Sparse<cols, tableSize, ElemT> >;
